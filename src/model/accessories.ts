...
/** 아바타별 팔레트 오버라이드 — 기본값은 플라밍고 트랙재킷 색 (기존 룩 불변) */
export interface AccessoryColors {
  bandBase: number; bandBaseShade: number   // 손목밴드 베이스
  bandStripe: number; bandStripeShade: number // 손목밴드 굵은 스트라이프
  bandLine: number; bandLineShade: number   // 손목밴드 핀라인
  cord: number; cordShade: number           // 드로스트링 코드
  tip: number; tipShade: number             // 드로스트링 팁(aglet)
}

export const DEFAULT_ACC_COLORS: AccessoryColors = {
  bandBase: ACC_COL.white, bandBaseShade: ACC_COL.whiteShade,
  bandStripe: ACC_COL.navy, bandStripeShade: ACC_COL.navyShade,
  bandLine: ACC_COL.coral, bandLineShade: ACC_COL.coralShade,
  cord: ACC_COL.coral, cordShade: ACC_COL.coralShade,
  tip: ACC_COL.navy, tipShade: ACC_COL.navyShade,
}
 ...
 */
function buildWristband(hand: THREE.Object3D, lowerArm: THREE.Object3D, col: AccessoryColors): void {
...
function buildString(side: -1 | 1, hw: number, col: AccessoryColors): StringRig {
  ...
  const cord = new THREE.Mesh(
    ...
    toonMat(col.cord, col.cordShade),
  )
