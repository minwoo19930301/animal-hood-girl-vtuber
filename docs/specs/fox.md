...
| 항목 | 실제 구현 |
|---|---|
| 헤어 | 플래티넘 실버 base `#D9DCE3` / shade `#98A3B3` / accent `#E8E9EE` — v3.1(러스트 후드와 강대비). 렌더 보상 틴트 `#F6F8FC`. 하단 컷 row ≈737 (울프컷 레이어드) |
| 홍채 | 에메랄드 3단 램프 `#174A35 / #4FA46C / #D2F0A8` |
| 눈매(v3.1) | 날카로운 폭스아이: lift +10 / thick 1.2 / lower 0.2 / irisScale 0.90 / highlight 0.82x / brow sharp-arch(틸트 +3) / 바깥 속눈썹 강조. expressionBias angry 0.12 |
...
- **삼각 귀 ×2**: 납작 콘(반폭 0.30·crownH, 높이 0.62·crownH, z 0.45) + 상단 42%를
  ...
  높이 밴드 [0.44, 0.60]·earH 클램프 — 판정 라운드: 기존 0.47±0.105 밴드는 돔
  교차선(~0.10L) 위 마진이 0.025L뿐이라 미세 요에도 좌우 노출이 비대칭 슬리버로
  갈라졌다. 하단 마진 0.07L 확보로 좌우 동일 노출, 상단은 다크 팁 아래 심 은폐).
  anchor azimuth ±0.70 / elevation 0.80, 위끝 바깥 벌어짐.
- **브로우 터프트 ×2**: 눈썹 위쪽 셸 면 surfacePoint(±0.30, 0.56, radial 0.965) —
  개구부 콘 밖(콘축 이격 θ≈0.83). 판정 라운드: ±0.36/0.985 + 들림 0.30은 orbit에서
  셸 실루엣 밖으로 삐침 — 안쪽 이동+인셋+들림 0.24·부채 벌어짐 축소.
