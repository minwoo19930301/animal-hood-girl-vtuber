...
| 스트라이프 | 웜블랙 `#2E2620` **unlit 플랫** 곡선 스트라이프 (판정 라운드: 토온 셰이드·림이 각진 회색 스페큘러로 읽혀 unlitMat으로 교체, 아웃라인 제거 — 프린트 마킹 룩) — 좌우 대칭 4쌍(P1 이마 八자 0.45/0.86 · P2 볼 옆 1.30/0.32 · P3 상측면 1.62/0.72 · P4 뒤통수 2.48/0.55, 폭 0.062~0.070L) + 정수리 세로 자오선 1개(폭 0.075L). taperedTube(seg 28, radial 12, scaleY 0.30)를 셸 곡률 호(y=ρsinφ, z=ρ(cosφ−1))로 구부려 radial 0.985 인셋 밀착, 끝 라운드 테이퍼([0.55, 0.95, 1, 0.95, 0.55]·wid) — pitch 0.4 셸 실루엣 돌출 0. **안감 관통 주의(재판정 사고 이력)**: 인셋+아크 끝 딥 때문에 튜브 안쪽 면이 안감(0.985R)을 뚫는다 — 관통부가 정면 개구부 너머로 보이는 위치(구 P4 2.35/0.42)에 두면 화이트 림 밴드 위 검정 삼각 얼룩으로 비친다(ablation으로 P4 확정, 볼 스트라이프 오인 주의). 후면 스트라이프는 el ≥0.5·az ≥2.4대(머리 가림 영역)에 배치할 것 |
