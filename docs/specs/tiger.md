...
| 스트라이프 | 웜블랙 `#2E2620` **unlit 플랫** 곡선 스트라이프 (판정 라운드: 토온 셰이드·림이 각진 회색 스페큘러로 읽혀 unlitMat으로 교체, 아웃라인 제거 — 프린트 마킹 룩) — 좌우 대칭 4쌍(이마 八자·볼 옆·상측면·뒤통수, 폭 0.062~0.070L) + 정수리 세로 자오선 1개(폭 0.075L). taperedTube(seg 28, radial 12, scaleY 0.30)를 셸 곡률 호(y=ρsinφ, z=ρ(cosφ−1))로 구부려 radial 0.985 인셋 밀착, 끝 라운드 테이퍼([0.55, 0.95, 1, 0.95, 0.55]·wid) — pitch 0.4 셸 실루엣 돌출 0 |
