...
| 항목 | 실제 계약 |
|---|---|
| 헤어 | 밀크티 베이지 base `#C7AD8E` / shade `#996E4C` / accent `#E9DFD2` — v3.1(흑백 의상 위 웜 팝). 렌더 보상 틴트 `#DFC9AB`. 컷 없음 — 롱 실루엣 유지. |
| 홍채 | 회보라 3단: `#3B1F3E` / `#8B568C` / `#EBCFEA`. eyeSharpen 카탈로그 0.32 → 런타임 클램프 후 0.070 (가장 느긋한 눈매 그룹). |
| 눈매(v3.1) | 동글 순둥: lift -3 / thick 1.05 / lower 0.55 / irisScale 1.10 / highlight 1.12x / brow thick-short. 눈썹 틴트 헤어 동조. expressionBias relaxed 0.08 |
...
`cordScale 0.62` — 기본 길이(1.12hw)는 레드 코드가 레드 보타이 위를 가로질러
레드-온-레드로 겹쳤다(판정 라운드). 코드가 보타이 위에서 끝나도록 판다만 축소
(buildAccessories 4번째 옵션 인자, 타 종 기본 1 무영향).
