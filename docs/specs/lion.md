...
| 갈기 바깥 겹 | 앰버 `#C8872E/#9C6220` featherLobe×11, δ±2.05(림 상단 기준, 턱 아래 비움), margin 0.34, lift 0.35, len 0.55·crownH. 크기 변주는 인덱스 코사인(결정적) |
| 갈기 안쪽 겹 | 허니 `#F7D180/#D9A54A` featherLobe×10, δ±1.85+stagger 0.10, margin 0.14, lift 0.60, len 0.36·crownH |
| 둥근 귀×2 | 림 프레임 앵커 `rimAnchor(δ±0.82, margin 0.30)` — 안겹 페탈 사이 틈. 셸색 돔(R 0.26·crownH, z-lift 0.16·crownH로 페탈 층 위) + 앰버 이너 디스크 `#B5722E/#8A5520`. hitMeshes 등록 |
