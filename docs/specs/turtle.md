...
- **등껍질 플레이트**: 다크그린 `0x4C7038/0x30491F` 납작 육각 실린더 디스크 10개
  ...
  (az 0, el 1.02, s 0.56, tx 0.52) + 정면 상측 좌우 (az ±1.12, el 0.74, s 0.66,
  tx 0.22, ty ±0.50). 방사 법선 그대로는 정면에서 엣지온 탭으로 읽혀(재판정 P2)
  tx/ty 페이스-틸트 래퍼 그룹으로 디스크 법선을 정면 카메라 쪽으로 돌렸다
  (n·(−ẑ) ≈ 0.75~0.95). 틸트 판은 z 오프셋 h·0.45로 앞 에지 잠김을 상쇄 —
  들리는 에지는 림 반대쪽(배갑 스큐트 단차로 판독). 개구부 콘축 이격 θ ≈1.24/1.40
  rad vs 해당 φ 콘 경계 ≈0.58/0.75 — 판 각반경(≤0.17) 감안 마진 ≥0.45 rad
  (틸트는 앵커 위치 불변이라 마진 유지). 전부 surfacePoint 앵커(radial 1.0),
  roll은 결정적 고정값.
